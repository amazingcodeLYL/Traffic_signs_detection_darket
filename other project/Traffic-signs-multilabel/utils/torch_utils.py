...
def load_classifier(name='resnet50', n=1000):
    # Loads a pretrained model reshaped to n-class output
    # import pretrainedmodels  # https://github.com/Cadene/pretrained-models.pytorch#torchvision
    # model = pretrainedmodels.__dict__[name](num_classes=1000, pretrained='imagenet')
    import torchvision
    model = torchvision.models.__dict__[name](pretrained=True,num_classes=1000)
    ...
    print(model.fc.weight.shape) #[1000, 2048]
    filters = model.fc.weight.shape[1]
    model.fc.bias = torch.nn.Parameter(torch.zeros(n))
    model.fc.weight = torch.nn.Parameter(torch.zeros(n,filters)) #[44, 2048]
    model.fc.out_features = n
    dim_feats=model.fc.in_features
    model.fc=nn.Linear(dim_feats,n)
    print(model)
