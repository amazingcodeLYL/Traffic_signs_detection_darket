...
def load_classes(path):
    ...
    with open("data/tt100k.names") as f:
        names = f.read().split('\n')
...
def get_yolo_layers(model):
    ...
    return [i for i, x in enumerate(bool_vec) if x]  # [82, 94, 106] for Traffic-signs-multilabel
...
def strip_optimizer(f='weights/best_ones.pt'):  # from utils.utils import *; strip_optimizer()
...
def create_backbone(f='weights/best_ones.pt'):  # from utils.utils import *; create_backbone()
